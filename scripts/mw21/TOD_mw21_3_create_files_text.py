...
#@@@ Sample command @@@#
# export HF_DATASETS_OFFLINE=1 ;export TRANSFORMERS_OFFLINE=1 ;export HF_EVALUATE_OFFLINE=1; export CUDA_VISIBLE_DEVICES=$(free-gpus.sh 1); python TOD_mw21_3_create_files_text.py --tsv_npy_creation Y --punct Y --dialog_side U
# export HF_DATASETS_OFFLINE=1 ;export TRANSFORMERS_OFFLINE=1 ;export HF_EVALUATE_OFFLINE=1; export CUDA_VISIBLE_DEVICES=$(free-gpus.sh 1); python TOD_mw21_3_create_files_text.py --tsv_npy_creation N --punct N --dialog_side U
# export HF_DATASETS_OFFLINE=1 ;export TRANSFORMERS_OFFLINE=1 ;export HF_EVALUATE_OFFLINE=1; export CUDA_VISIBLE_DEVICES=$(free-gpus.sh 1); python TOD_mw21_3_create_files_text.py --tsv_npy_creation N --punct Y --dialog_side UA
# export HF_DATASETS_OFFLINE=1 ;export TRANSFORMERS_OFFLINE=1 ;export HF_EVALUATE_OFFLINE=1; export CUDA_VISIBLE_DEVICES=$(free-gpus.sh 1); python TOD_mw21_3_create_files_text.py --tsv_npy_creation N --punct N --dialog_side UA
...
def parse_args():
    ...
    # parser.add_argument("--output_file", type=str, default='LLM_input_text_U.txt')
    parser.add_argument("--data_path", type=str, default='/mnt/matylda4/hegde/int_ent/TOD_llm/Data/Multiwoz2.1/')
    parser.add_argument("--output_folder", type=str, default='/mnt/matylda4/hegde/int_ent/TOD_llm/experiments/mw21/NecessaryFiles_text/')
    # parser.add_argument("--output_folder", type=str, default='/mnt/matylda4/hegde/int_ent/TOD_llm/experiments/mw21/NecessaryFiles_text_NP/')
...
def main():
    ...
    print(f" tsv_npy_creation: {args.tsv_npy_creation}")
    ...
    if args.dialog_side == 'U':
        nearest_neighbour_file_name = os.path.join(output_folder_path, 'LLM_input_text_U.txt')
        if args.punct == 'N':
            nearest_neighbour_file_name = os.path.join(output_folder_path, 'LLM_input_text_U_PN.txt')
        ...
        if args.tsv_npy_creation == 'Y':
            ...
            train_numpy_file_path = calculate_save_sentence_embedding(train_tsv_file_path, 'U', punct='punct')  # Let the punct be 'Y'. If it is no in the argument, later it will be removed. Need not worry here. 
            test_numpy_file_path = calculate_save_sentence_embedding(test_tsv_file_path, 'U', punct='punct')    # So that making the tsv and numpy files again will not be required.
    
    elif args.dialog_side == 'UA':
        nearest_neighbour_file_name = os.path.join(output_folder_path, 'LLM_input_text_UA.txt')
        if args.punct == 'N':
            nearest_neighbour_file_name = os.path.join(output_folder_path, 'LLM_input_text_UA_PN.txt')
