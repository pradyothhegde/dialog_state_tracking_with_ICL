...
#@@@ Sample command @@@#
# export HF_DATASETS_OFFLINE=1 ;export TRANSFORMERS_OFFLINE=1 ;export HF_EVALUATE_OFFLINE=1; export CUDA_VISIBLE_DEVICES=$(free-gpus.sh 1); python TOD_mw21_3_create_files_speech_mix.py --tsv_npy_creation Y --punct Y --dialog_side U
# export HF_DATASETS_OFFLINE=1 ;export TRANSFORMERS_OFFLINE=1 ;export HF_EVALUATE_OFFLINE=1; export CUDA_VISIBLE_DEVICES=$(free-gpus.sh 1); python TOD_mw21_3_create_files_speech_mix.py --tsv_npy_creation N --punct N --dialog_side U
# export HF_DATASETS_OFFLINE=1 ;export TRANSFORMERS_OFFLINE=1 ;export HF_EVALUATE_OFFLINE=1; export CUDA_VISIBLE_DEVICES=$(free-gpus.sh 1); python TOD_mw21_3_create_files_speech_mix.py --tsv_npy_creation N --punct Y --dialog_side UA
# export HF_DATASETS_OFFLINE=1 ;export TRANSFORMERS_OFFLINE=1 ;export HF_EVALUATE_OFFLINE=1; export CUDA_VISIBLE_DEVICES=$(free-gpus.sh 1); python TOD_mw21_3_create_files_speech_mix.py --tsv_npy_creation N --punct N --dialog_side UA
import os
import numpy as np
...
import ast
from TODx import finding_test_labels, json_to_tsv_format, calculate_save_sentence_embedding, remove_punctuations, json_to_tsv_format_asrTrans_mix
import json
import argparse
import torch
...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, default='/mnt/matylda4/hegde/int_ent/TOD_llm/Data/Multiwoz2.1/')
    parser.add_argument("--output_folder", type=str, default='/mnt/matylda4/hegde/int_ent/TOD_llm/experiments/mw21/NecessaryFiles_speech')
    # parser.add_argument("--output_folder", type=str, default='/mnt/matylda4/hegde/int_ent/TOD_llm/experiments/mw21/NecessaryFiles_speech_NP')
    ...
    parser.add_argument("--dialog_side", type=str, default='U')
    parser.add_argument("--tsv_npy_creation", type=str, default='Y', help="Y=yes, N=no")
    parser.add_argument("--punct", type=str, default='Y', help="Y=yes, N=no")      # have different folder for punct and no punct
...
def main():
    ...
    input_test_data_path = os.path.join(data_path, 'mw21_DST_test.json')
    input_train_data_path = os.path.join(data_path, 'mw21_DST_train.json')
    human_trans_sorted_file = os.path.join(data_path, 'text_half_human_sorted')
    ...
    if args.dialog_side == 'U':
        nearest_neighbour_file_name = os.path.join(output_folder_path, 'LLM_input_speech_U.txt')
        if args.punct == 'N':
            nearest_neighbour_file_name = os.path.join(output_folder_path, 'LLM_input_speech_U_PN.txt')

        test_tsv_file_path = os.path.join(output_folder_path, 'mw21_DST_test_U_punct_turns.tsv')
        train_tsv_file_path = os.path.join(output_folder_path, 'mw21_DST_train_U_turns.tsv')
        test_numpy_file_path = os.path.join(output_folder_path, 'mw21_DST_test_U_punct_turns.npy')
        train_numpy_file_path = os.path.join(output_folder_path, 'mw21_DST_train_U_turns.npy')

        if args.tsv_npy_creation == 'Y':
            json_to_tsv_format(input_train_data_path, train_tsv_file_path, 'U')    # Let the punct be 'Y'. If it is no in the argument, later it will be removed. Need not worry here. 
            json_to_tsv_format_asrTrans_mix(input_test_data_path, test_tsv_file_path, dialog_side, human_trans_sorted_file, punct='Y')      # So that making the tsv and numpy files again will not be required.
        ...
        nearest_neighbour_file_name = os.path.join(output_folder_path, 'LLM_input_speech_UA.txt')
        if args.punct == 'N':
            nearest_neighbour_file_name = os.path.join(output_folder_path, 'LLM_input_speech_UA_PN.txt')

        test_tsv_file_path = os.path.join(output_folder_path, 'mw21_DST_test_UA_punct_turns.tsv')
        train_tsv_file_path = os.path.join(output_folder_path, 'mw21_DST_train_UA_turns.tsv')
        test_numpy_file_path = os.path.join(output_folder_path, 'mw21_DST_test_UA_punct_turns.npy')
        train_numpy_file_path = os.path.join(output_folder_path, 'mw21_DST_train_UA_turns.npy')

        if args.tsv_npy_creation == 'Y':
            json_to_tsv_format(input_train_data_path, train_tsv_file_path, 'UA')
            json_to_tsv_format_asrTrans_mix(input_test_data_path, test_tsv_file_path, dialog_side, human_trans_sorted_file, punct='Y')
    ...
    if args.dialog_side == 'U':
        test_file_path = os.path.join(output_folder_path, 'mw21_DST_test_U_punct_turns.tsv')
        test_numpy_path = os.path.join(output_folder_path, 'mw21_DST_test_U_punct_turns.npy') 
        train_file_path = os.path.join(output_folder_path, 'mw21_DST_train_U_turns.tsv')
        train_numpy_path = os.path.join(output_folder_path, 'mw21_DST_train_U_turns.npy')
    elif args.dialog_side == 'UA':
        test_file_path = os.path.join(output_folder_path, 'mw21_DST_test_UA_punct_turns.tsv')
        test_numpy_path = os.path.join(output_folder_path, 'mw21_DST_test_UA_punct_turns.npy') 
        train_file_path = os.path.join(output_folder_path, 'mw21_DST_train_UA_turns.tsv')
        train_numpy_path = os.path.join(output_folder_path, 'mw21_DST_train_UA_turns.npy')
