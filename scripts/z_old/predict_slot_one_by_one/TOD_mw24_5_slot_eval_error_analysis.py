...
def compare_slots(gold_slots, predicted_slots, domain_slots):
    ...
    for slot in domain_slots:
        if slot in gold_slots:
            ...
            # print(type(gold_value))
            # print(type(predicted_slots[slot]))
            # print(TODx.remove_punctuations([predicted_slots[slot]])[0])
            gold_value = TODx.remove_punctuations([gold_value])[0]
            predicted_slot = TODx.remove_punctuations([predicted_slots[slot].strip()])[0]
            if gold_value == "na":
                total_NA_count += 1
                # Count as correct if slot is absent in predicted_slots
                # print("NA occured")
                if gold_value == predicted_slot:
                    # print("correct NA")
                    correct_count += 1 
                    correct_NA_count += 1
                ...
                if slot in predicted_slots and gold_value == predicted_slot: # if there is slot of same name in prediction, and the value is same
    ...
    for file in os.listdir(eval_file):
        if file.endswith('.json'):
            ...
            for line in eval_data:
                ...
                # print(f"Gold Domain: {gold_domain}")
                ...
                for g_domain, g_slots in zip(gold_domain, gold_slots):
                    if g_domain in predicted_output:
                        ...
                        # print(
                        #     f"Domain: {g_domain}, Correct: {correct}, Total: {total}"
                        # )
                        # print(f"predicted slots: {predicted_slots}")
                        # breakpoint()
                # print("=============")
