...
TextProcessor = TODx_add_punctuation_clean.TextProcessor
...
def json_to_tsv_format(input_data_path, tsv_file_path, dialog_side, punct):
    ...
    text_processor = TextProcessor("pcs_en")
        ...
        for line in tqdm(input_file):
            ...
            for fname in line_data:
                ...
                for i, log_entry in enumerate(log):
                    ...
                    if punct == 'N':
                        text = remove_punctuations([text])[0]
                    elif punct == 'R':
                        text = remove_punctuations([text])[0]
                        # import from TODx_add_punctuation_clean, which has class TextProcessor and function process_line within it.
                        text = text_processor.process_line(text)
...
def calculate_save_sentence_embedding(tsv_file_path, dialog_side, punct):    # file, dialog_side, punct
    ...
    # elif punct == 'R':        # skipping this because the tsv file is already processed with repunctuation.
    ...
    #     new_sentences = []
        ...
        #     new_sentence = text_processor.process_line(sentence)
