...
def arg_parse():
    ...
    parser.add_argument(
        '--eval_folder_path',
        type=str,
        default='/mnt/matylda4/hegde/int_ent/TOD_llm/experiments/mw24/O7BI/usr_sgnt_ualimtd_TOD_mw24_O7BI_SD_I1_U_10_PY_TY',
...
def compare_slots(gold_slots, predicted_slots, domain_slots, domain):
    ...
    for slot in domain_slots:
        ...
        # print(f"Predicted Value: {predicted_slots[slot]}")
        try:
            # print(f"Predicted Value: {predicted_slots[slot]}")
            predicted_value = TODx.remove_punctuations([str(predicted_slots.get(slot, "na"))])[0]
        ...
        # in the prediction, the slot maybe missing.

        if gold_value == "na":
            # Check if the slot name itself is missing in the prediction
            if (slot not in predicted_slots) or (predicted_value == "na") or (predicted_value == ""):
                tn += 1
            ...
            # If the slot is present in the prediction and the value is equal to the gold value
